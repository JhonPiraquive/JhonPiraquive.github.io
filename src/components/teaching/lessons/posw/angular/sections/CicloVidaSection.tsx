...
export function CicloVidaSection() {
  return (
    <section>
      <h2 className="mb-4 text-2xl font-bold text-[var(--color-primary)]">{"Ciclo de vida"}</h2>
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Mapa mental"}</h3>
      <ul className="my-4 list-disc pl-6">
        <li>{"ngOnChanges: cuando cambian @Input()."}</li>
        <li>{"ngOnInit: inicialización tras primer ngOnChanges; ideal para fetch inicial."}</li>
        <li>{"ngAfterViewInit: tras renderizar la vista del componente."}</li>
        <li>{"ngOnDestroy: limpieza antes de destruir; cancelar suscripciones."}</li>
        <li>{"Regla: siempre limpiar Observables en ngOnDestroy o usar async pipe."}</li>
      </ul>
      <StepReveal
