...
export function ComponentesSection() {
  return (
    <section>
      <h2 className="mb-4 text-2xl font-bold text-[var(--color-primary)]">{"Componentes"}</h2>
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Mapa mental"}</h3>
      <ul className="my-4 list-disc pl-6">
        <li>{"Componente: unidad básica — clase TypeScript + template HTML + estilos CSS opcionales."}</li>
        <li>{"@Component: selector, templateUrl, styleUrls."}</li>
        <li>{"@Input(): datos del padre al hijo (solo lectura en el hijo)."}</li>
        <li>{"@Output(): eventos del hijo al padre (EventEmitter)."}</li>
        <li>{"Árbol de componentes: jerarquía AppComponent → features → presentacionales."}</li>
      </ul>
      <CodeFiddle
        language="typescript"
