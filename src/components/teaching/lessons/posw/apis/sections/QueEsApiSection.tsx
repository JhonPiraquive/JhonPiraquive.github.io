...
export function QueEsApiSection() {
  return (
    <section>
      <h2 className="mb-4 text-2xl font-bold text-[var(--color-primary)]">{"¿Qué es una API?"}</h2>
      ...
      <ul className="my-4 list-disc pl-6">
        ...
        <li>{"Abstracción: oculta la implementación interna (lenguaje, BD, arquitectura)."}</li>
        <li>
          {
      ...
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Definición y analogía del menú"}</h3>
      <p className="my-4">
        {
      ...
      <ul className="my-4 list-disc pl-6">
        ...
        <li>{"Menú (contrato): endpoints documentados con método, URI y formato de respuesta."}</li>
      </ul>
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Flujo básico cliente → API → datos"}</h3>
      <MermaidDiagram
  ...
  A-->>C: 200 OK + JSON`}
      />
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Ejemplo: consulta a API pública"}</h3>
      <CodeFiddle
        language="http"
