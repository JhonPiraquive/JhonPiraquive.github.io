...
export function QueEsBackendSection() {
  return (
    <section>
      <h2 className="mb-4 text-2xl font-bold text-[var(--color-primary)]">{"¿Qué es el backend?"}</h2>
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Mapa mental"}</h3>
      <ul className="my-4 list-disc pl-6">
        <li>{"Backend (server-side): código que corre en el servidor, invisible para el usuario final."}</li>
        <li>{"Responsabilidades centrales: lógica de negocio, persistencia, autenticación, integraciones."}</li>
        <li>{"Contrato con el frontend: expone APIs (REST, GraphQL, gRPC) que el cliente consume."}</li>
        <li>{"Analogía restaurante: frontend = salón y menú; backend = cocina; API = mesero que traduce pedidos."}</li>
      </ul>
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Analogía del restaurante"}</h3>
      <p className="my-4">{"En una app de reservas de hotel:"}</p>
      <ul className="my-4 list-disc pl-6">
        <li>{"Frontend (salón): formularios, calendario, UX."}</li>
        <li>{"API (mesero): traduce GET /api/v1/habitaciones a operaciones internas."}</li>
        <li>{"Backend (cocina): valida disponibilidad, consulta BD, aplica reglas de precio y cancelación."}</li>
      </ul>
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Arquitectura en capas"}</h3>
      <MermaidDiagram
        chart={`flowchart TD
