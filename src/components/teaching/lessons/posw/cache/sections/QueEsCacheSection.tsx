...
export function QueEsCacheSection() {
  return (
    <section>
      <h2 className="mb-4 text-2xl font-bold text-[var(--color-primary)]">{"¿Qué es la caché?"}</h2>
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Mapa mental"}</h3>
      <ul className="my-4 list-disc pl-6">
        <li>{"Caché: guarda copias de datos costosos para servir solicitudes idénticas más rápido."}</li>
        <li>{"Principio: si ya calculaste o buscaste algo, guarda el resultado y reutilízalo."}</li>
        <li>{"Cache Hit Rate: (Hits / Total Requests) × 100; objetivo en producción > 90%."}</li>
        <li>{"Latencia: sin caché 100–500 ms (BD/red); con Redis < 1 ms."}</li>
      </ul>
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Analogía de apuntes"}</h3>
      <p className="my-4">
        {
      ...
      />
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Práctica: calcular hit rate"}</h3>
      <PracticeExercise
