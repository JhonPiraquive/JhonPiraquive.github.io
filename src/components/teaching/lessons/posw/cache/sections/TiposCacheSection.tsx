...
export function TiposCacheSection() {
  return (
    <section>
      <h2 className="mb-4 text-2xl font-bold text-[var(--color-primary)]">{"Tipos de caché"}</h2>
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Mapa mental"}</h3>
      <ul className="my-4 list-disc pl-6">
        <li>{"Browser Cache: assets estáticos en disco local; controlado por headers HTTP."}</li>
        <li>{"Server Cache: resultados de operaciones costosas; Redis, Memcached."}</li>
        <li>{"CDN Cache: servidores globales cerca del usuario; Cloudflare, CloudFront."}</li>
        <li>{"Database Cache: buffer pool interno, vistas materializadas."}</li>
      </ul>
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Arquitectura de tipos"}</h3>
      <MermaidDiagram
        chart={`flowchart TD
  ...
  API --> DB[(Database Cache / BD)]`}
      />
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Comparativa por tipo"}</h3>
      <CompareTable
