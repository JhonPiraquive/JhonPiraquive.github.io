...
export function MetodosHttpSection() {
  return (
    <section>
      <h2 className="mb-4 text-2xl font-bold text-[var(--color-primary)]">{"Métodos HTTP (verbos)"}</h2>
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Mapa mental"}</h3>
      <ul className="my-4 list-disc pl-6">
        <li>{"Los métodos indican la acción sobre un recurso en una URI."}</li>
        <li>{"Semántica definida en RFC 9110."}</li>
        <li>{"Propiedades clave: Safe (no cambia estado) e Idempotente (repetir = mismo efecto)."}</li>
        <li>{"GET, HEAD, OPTIONS son Safe; POST y PATCH en general no son idempotentes."}</li>
      </ul>
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Qué son los métodos HTTP"}</h3>
      <p className="my-4">
        {
      ...
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Métodos principales"}</h3>
      <CompareTable
      ...
      />
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Ejemplo: GET vs POST"}</h3>
      <CodeFiddle
        language="http"
        title="GET producto"
...
{"nombre": "Teclado mecánico", "precio": 320000}`}
      />
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Safe e Idempotente explicados"}</h3>
      <ul className="my-4 list-disc pl-6">
        <li>
          {
