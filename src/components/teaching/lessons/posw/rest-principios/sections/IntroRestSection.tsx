...
export function IntroRestSection() {
  return (
    <section>
      <h2 className="mb-4 text-2xl font-bold text-[var(--color-primary)]">{"Introducción a REST"}</h2>
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Mapa mental"}</h3>
      <ul className="my-4 list-disc pl-6">
        <li>
          {"REST (Representational State Transfer): estilo arquitectónico de Roy Fielding (tesis 2000)."}
        </li>
        <li>{"No es protocolo ni estándar — se implementa sobre HTTP en la práctica."}</li>
        <li>{"Seis constraints definen si un sistema es RESTful."}</li>
        <li>{"Modelo de Richardson: niveles 0–3 de madurez REST."}</li>
      </ul>
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Los seis constraints"}</h3>
      <MermaidDiagram
        chart={`flowchart TD
  ...
  S5 --> U4[HATEOAS]`}
      />
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Modelo de madurez Richardson"}</h3>
      <CompareTable
      ...
      />
      <h3 className="mt-6 mb-2 text-xl font-semibold">{"Recurso vs acción (anti-patrón)"}</h3>
      <CodeFiddle
        language="http"
