...
export function EjemploTecnicoQueObservariasSection() {
  return (
    <section>
      ...
      <CodeBlock className="language-bash">{`# Buscar accesos a rutas administrativas y picos por IP (ejemplo conceptual).
# Ajusta rutas/archivos según tu servidor (nginx/apache/app).
grep -E &quot;/admin|/login&quot; /var/log/nginx/access.log | tail -n 50
awk &#x27;{print \$1}&#x27; /var/log/nginx/access.log | sort | uniq -c | sort -nr | head -n 10`}</CodeBlock>
      <CodeBlock className="language-http">{`# Patrón de requests repetitivas a un endpoint sensible (ejemplo conceptual)
...
GET /admin/export?format=csv HTTP/1.1
Host: ejemplo.com
User-Agent: Mozilla/5.0`}</CodeBlock>
    </section>
  );
}
